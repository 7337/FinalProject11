...
if(isset($_GET['add'])){
	...
	while($qt_row = mysql_fetch_assoc($qt)){
		if($qt_row['br_stok'] != $_SESSION['cart_'.$_GET['add']] && $qt_row['br_stok'] > 0){
            ...
            echo '<script language="javascript">alert("Produk berhasil dimasukan ke keranjang anda.."); document.location="detail.php";</script>';
            
		} else {
			echo '<script language="javascript">alert("Stok produk tidak mencukupi!"); document.location="produk.php";</script>';
		}
	}
